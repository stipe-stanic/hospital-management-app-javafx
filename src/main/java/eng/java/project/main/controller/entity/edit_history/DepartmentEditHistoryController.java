...
public class DepartmentEditHistoryController {
    ...
    private static final Logger logger = LoggerFactory.getLogger(DepartmentEditHistoryController.class);
    private List<EditInfo<CoreObject, GlobalUser>> editInfoList = new ArrayList<>();
    private static final String SERIALIZED_FILE_NAME_DEPARTMENT = "files\\edited_department_object.dat";

    public void initialize() {
        try(ObjectInputStream in = new ObjectInputStream(new FileInputStream(SERIALIZED_FILE_NAME_DEPARTMENT))) {
            try {
        ...
        entityColumn.setCellValueFactory(data -> {
            ...
            if (afterEditObject instanceof Department department) {
